...
class Split{
    ...
    public Split(String user,double amt){
        this.UserId=user;
        this.amount=amt;
    }
}
...
interface SplitStrategy{
    List<Split> calculateSplit(double totalamt,List<String> userIds,List<Double> values);
}
class EqualSplit implements SplitStrategy{
    public List<Split> calculateSplit(double totalamt,List<String> userIds,List<Double> values){
        List<Split> splits=new ArrayList<>();
        double amtperuser=totalamt/userIds.size();
        for(String id:userIds){
            splits.add(new Split(id, amtperuser));
        }
        return splits;
    }
}
...
class DebtSimplifier{
        ...
        for(Map.Entry<String,Map<String,Double>> userBalance:groupBalances.entrySet()){
            String creditorId=userBalance.getKey();
             for (Map.Entry<String, Double> balance : userBalance.getValue().entrySet()) {
                ...
                Double amt=balance.getValue();
                //only process positive amt to remove double ccounting
                if(amt>0){
                    netAmounts.put(creditorId,netAmounts.get(creditorId)+amt);
                    netAmounts.put(debtorId,netAmounts.get(debtorId)-amt);
                }
             }
        }
 ...
 class Group{
    ...
    public Map<String, Map<String, Double>> groupBalances;
